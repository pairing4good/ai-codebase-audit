...
2. Generate ALL required artifacts in $PROJECT_ROOT/.analysis/python/stage1-artifacts/:
...
Output all files to $PROJECT_ROOT/.analysis/python/stage1-artifacts/
```

4. After the agent completes, read `$PROJECT_ROOT/.analysis/python/stage1-artifacts/architecture-overview.md` and present a summary to the user
...
Read Stage 1 artifacts from $PROJECT_ROOT/.analysis/python/stage1-artifacts/ for context.
...
Output your findings to: $PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/architecture-analysis.json
...
Output your findings to: $PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/security-analysis.json
...
Output your findings to: $PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/maintainability-analysis.json
...
Output your findings to: $PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/dependency-analysis.json
...
5. Generate a convergence preview by identifying findings that appear in multiple agent outputs:
   ...
   - Write `$PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/convergence-preview.md` showing multi-agent findings
   - Write `$PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/metadata.json` with counts
...
```bash
node .claude/skills/audit-python/tools/format-static-results.js $PROJECT_ROOT/.analysis/python/stage3-static-analysis
```

This creates:
- `$PROJECT_ROOT/.analysis/python/stage3-static-analysis/unified-results.json` (normalized format)
- `$PROJECT_ROOT/.analysis/python/stage3-static-analysis/tool-comparison.md` (which tools found what)
- `$PROJECT_ROOT/.analysis/python/stage3-static-analysis/overlap-analysis.json` (convergence across tools)

6. Read `tool-comparison.md` and present summary to user

7. Write `$PROJECT_ROOT/.analysis/python/stage3-static-analysis/metadata.json` with tool execution status
...
Your inputs:
- Stage 1 artifacts: $PROJECT_ROOT/.analysis/python/stage1-artifacts/ (Python architecture context)
- Stage 2 agent outputs: $PROJECT_ROOT/.analysis/python/stage2-parallel-analysis/*.json (4 independent analyses)
- Stage 3 static analysis: $PROJECT_ROOT/.analysis/python/stage3-static-analysis/unified-results.json (Python tool findings)
...
Output to $PROJECT_ROOT/.analysis/python/stage4-reconciliation/:
...
Your input:
- Reconciled findings: $PROJECT_ROOT/.analysis/python/stage4-reconciliation/reconciled-longlist.json
...
Output to $PROJECT_ROOT/.analysis/python/stage5-adversarial/:
...
3. Read `$PROJECT_ROOT/.analysis/python/stage5-adversarial/challenged-findings.json`
...
7. Write Stage 6 outputs:
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/prioritization-matrix.json` (all findings with scores)
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/top-10-detailed.json` (top 10 with full details)
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/honorable-mentions.md` (findings 11-20)
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/quick-wins.md` (low effort, high impact items)
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/systemic-patterns.md` (recurring Python issues)
   - `$PROJECT_ROOT/.analysis/python/stage6-final-synthesis/metadata.json` (statistics)
...
2. See `$PROJECT_ROOT/.analysis/python/final-report/FINDINGS-DETAILED.json` for complete structured data
3. See `$PROJECT_ROOT/.analysis/python/final-report/CONFIDENCE-MATRIX.md` for evidence transparency matrix
4. See `$PROJECT_ROOT/.analysis/python/` directory for complete stage-by-stage outputs
5. Consider running `/audit-python` again after fixes to measure improvement

## Full Details

All stage-by-stage outputs available in `$PROJECT_ROOT/.analysis/python/`:
...
**Executive Deliverables** (in $PROJECT_ROOT/.analysis/python/final-report/):
...
**Next Step**: Review `$PROJECT_ROOT/.analysis/python/final-report/ANALYSIS-REPORT.md` for your prioritized improvements.
```
